...
class LLMService:
    def __init__(self):
        ...
        if not self.api_key:
            logger.error("No API KEY detected. LLM features will be disabled.")
            return
    ...
    async def chat(
        ...
        if self.agent is None:
            raise RuntimeError("LLM service not initialized. Check ANTHROPIC_API_KEY.")
    ...
    async def parse_structured(
        self, user_prompt: str, system_prompt: str, output_schema: Type[T]
    ) -> T:
        if self.agent is None:
            raise RuntimeError("LLM service not initialized. Check ANTHROPIC_API_KEY.")
